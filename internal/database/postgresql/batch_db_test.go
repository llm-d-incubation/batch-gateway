...
func TestBatchStore(t *testing.T) {
	...
	t.Run("stores item successfully", func(t *testing.T) {
		...
		mock.ExpectExec("INSERT INTO "+testTable).
			WithArgs("batch-1", testTenantID, pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg(), pgxmock.AnyArg()).
...
func TestBatchGet(t *testing.T) {
	...
	t.Run("gets items by tenant ID with static", func(t *testing.T) {
		...
		rows := pgxmock.NewRows([]string{colID, colTenantID, colExpiry, colTags, colProcessorID, colPriority, colStatus, colSpec}).
			AddRow(item.ID, item.TenantID, item.Expiry, &tags, "", int64(0), item.Status, item.Spec)
	...
	t.Run("gets items without static data", func(t *testing.T) {
		...
		rows := pgxmock.NewRows([]string{colID, colTenantID, colExpiry, colTags, colProcessorID, colPriority, colStatus}).
			AddRow(item.ID, item.TenantID, item.Expiry, &tags, "", int64(0), item.Status)
	...
	t.Run("gets items by IDs", func(t *testing.T) {
		...
		rows := pgxmock.NewRows([]string{colID, colTenantID, colExpiry, colTags, colProcessorID, colPriority, colStatus, colSpec}).
			AddRow(item1.ID, item1.TenantID, item1.Expiry, &tags1, "", int64(0), item1.Status, item1.Spec).
			AddRow(item2.ID, item2.TenantID, item2.Expiry, &tags2, "", int64(0), item2.Status, item2.Spec)
