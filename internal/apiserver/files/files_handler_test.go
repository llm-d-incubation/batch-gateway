...
func setupTestHandler(t *testing.T) (*FilesApiHandler, *dbmock.MockBatchDBClient, *fsmock.MockBatchFilesClient, context.Context) {
	t.Helper()
	...
	config := &common.ServerConfig{
		FileMaxSizeBytes:             200 << 20, // 200 MB
		FileMaxLineCount:             50000,
		FileDefaultExpirationSeconds: 30 * 24 * 60 * 60, // 30 days
	}

	handler := NewFilesApiHandler(config, dbClient, filesClient)
...
func createTestFile(t *testing.T, handler *FilesApiHandler, ctx context.Context, filename, purpose, content string) openai.FileObject {
	t.Helper()
...
func doTestCreateFile(t *testing.T) {
	...
	storageLocation := fileObj.Filename
	fileReader, fileMeta, err := filesClient.Retrieve(ctx, storageLocation)
	if err != nil {
