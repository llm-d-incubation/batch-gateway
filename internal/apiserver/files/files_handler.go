...
func NewFilesApiHandler(config *common.ServerConfig, dbClient dbapi.BatchDBClient, filesClient fsapi.BatchFilesClient) *FilesApiHandler {
...
func (c *FilesApiHandler) dbItemToFileObject(item *dbapi.BatchItem) (*openai.FileObject, error) {
	fileSpec := &FileSpec{}
...
func (c *FilesApiHandler) getFileItemFromDB(ctx context.Context, w http.ResponseWriter, r *http.Request, operation string) *dbapi.BatchItem {
	...
	logger.V(logging.DEBUG).Info(operation+" file request", "file_id", fileID)
	...
	if err != nil {
		logger.Error(err, "failed to retrieve file metadata", "file_id", fileID)
		common.WriteInternalServerError(ctx, w)
		return nil
	}

	if len(items) == 0 {
		logger.Info("file not found", "file_id", fileID)
		apiErr := openai.NewAPIError(
			http.StatusNotFound,
			"",
...
func (c *FilesApiHandler) CreateFile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		_ = c.filesClient.Delete(ctx, fileMeta.Location)
		common.WriteInternalServerError(ctx, w)
		return
	}
	...
	tags := map[string]string{
		dbapi.TagKeyType:    dbapi.TagValueFile,
...
func (c *FilesApiHandler) ListFiles(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range items {
		...
		if err != nil {
			logger.Error(err, "failed to unmarshal file metadata", "file_id", item.ID)
			continue
		}
	...
	common.WriteJSONResponse(ctx, w, http.StatusOK, response)
}
...
func (c *FilesApiHandler) RetrieveFile(w http.ResponseWriter, r *http.Request) {
	...
	item := c.getFileItemFromDB(ctx, w, r, "retrieve")
	if item == nil {
		return
	}
	...
	fileObj, err := c.dbItemToFileObject(item)
	if err != nil {
		logger.Error(err, "failed to construct file object", "file_id", item.ID)
		common.WriteInternalServerError(ctx, w)
		return
	}
...
func (c *FilesApiHandler) DownloadFile(w http.ResponseWriter, r *http.Request) {
	...
	item := c.getFileItemFromDB(ctx, w, r, "download")
	if item == nil {
		return
	}
	...
	fileSpec := &FileSpec{}
	if err := json.Unmarshal(item.Spec, fileSpec); err != nil {
		logger.Error(err, "failed to unmarshal file spec", "file_id", item.ID)
		common.WriteInternalServerError(ctx, w)
		return
	}

	// Retrieve file content from storage
	fileReader, fileMeta, err := c.filesClient.Retrieve(ctx, fileSpec.Location)
	if err != nil {
		logger.Error(err, "failed to retrieve file content", "file_id", item.ID, "location", fileSpec.Location)
		common.WriteInternalServerError(ctx, w)
		return
	}
	defer fileReader.Close()
	...
	if err != nil {
		logger.Error(err, "failed to stream file content", "file_id", item.ID, "bytes_written", written)
		return
	}

	logger.Info("file download completed", "file_id", item.ID, "bytes_written", written)
}
...
func (c *FilesApiHandler) DeleteFile(w http.ResponseWriter, r *http.Request) {
	...
	item := c.getFileItemFromDB(ctx, w, r, "delete")
	if item == nil {
		return
	}
	...
	err := c.filesClient.Delete(ctx, fileSpec.Location)
	if err != nil {
		logger.Error(err, "failed to delete physical file", "file_id", item.ID, "location", fileSpec.Location)
	...
	if err != nil {
		logger.Error(err, "failed to delete file metadata", "file_id", item.ID)
		common.WriteInternalServerError(ctx, w)
		return
	}

	logger.V(logging.DEBUG).Info("file deleted successfully", "file_id", item.ID, "deleted_ids", deletedIDs)
	...
	common.WriteJSONResponse(ctx, w, http.StatusOK, deleteResp)
}
