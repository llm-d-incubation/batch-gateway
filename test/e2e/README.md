...
7. Creates NodePort services mapping to `https://127.0.0.1:8000` (apiserver) and `http://127.0.0.1:8081` (apiserver observability)
...
| Variable                  | Default                          | Description                                                |
|---------------------------|----------------------------------|------------------------------------------------------------|
| `TEST_APISERVER_URL`      | `https://127.0.0.1:8000`        | Base URL of the running API server (TLS)                   |
| `TEST_APISERVER_OBS_URL`  | `http://127.0.0.1:8081`         | Apiserver observability endpoint (health, metrics)         |
| `TEST_PROCESSOR_OBS_URL`  | auto-resolved by the e2e test helpers | Processor observability endpoint (health, metrics)   |
| `TEST_JAEGER_URL`         | `http://127.0.0.1:16686`        | Jaeger query endpoint for trace verification               |
...
```bash
TEST_APISERVER_URL=https://127.0.0.1:9000 TEST_TENANT_ID=my-tenant make test-e2e
```
